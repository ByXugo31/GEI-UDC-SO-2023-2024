...
void updateProcessList(Item *item){
    int status;
    int priority;
    pid_t pid1 = waitpid(item->posNum,&status,WNOHANG | WUNTRACED | WCONTINUED);
    priority=getpriority(PRIO_PROCESS,item->posNum);
    ...
    if(pid1==item->posNum){
    	    ...
    	    if(strcmp(NombreSenal(signal_number),"SIGUNKNOWN")==0) sprintf(item->data[4],"%d",signal_number);
...
int SustituirVariable(char * var, char * var2, char * valor, char *e[], List *EnvList) /*cambia una variable en el entorno que se le pasa como parámetro*/
{                                                        /*lo hace directamente, no usa putenv*/
    int pos;
    char *aux;
...
void authors(char *arg) {
    ...
    if (arg==NULL || strcmp((const char *) arg,"\0")==0){
        ...
        printf("- Hugo   @ByXugo31 -> GITHUB\n");
        return;
    }
    //Si el argumento es -l...
    if (strcmp(arg,"-l")==0) {
        //Se imprimen los logins.
        printf("- @ByXugo31 -> GITHUB\n");
        return;
    }
...
void help(char *arg) {
    ...
    struct command *comnds=initializeCommands(NUM_COMMANDS);
    //Si el argumento es nulo o vacio.
    if (arg==NULL || strcmp((const char *) arg,"\0")==0) {
        //Imprime la lista.
        printAllCmds(comnds);
...
void execForeground(char *tr[],bool background,cmd *cmds,List *h,List *f,List *m,List *p,List *env){
    pid_t pid=fork();
...
void execExternal(char *tr[],cmd *cmds,List *h,List *f,List *m,List *p,List *env){
    ...
    execForeground(tr,background,cmds,h,f,m,p,env);
}